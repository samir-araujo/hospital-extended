...
public class IncidentScheduler : MapComponent
{
    ...
    public override void MapComponentTick()
    {
        ...
        if (GenTicks.TicksGame % 42 == 0) // every ingame minute
        {
            ...
            float baseChance = 0.04f; // 10% every minute, gives about a patient per 100 minutes  
            if (GenLocalDate.DayOfQuadrum(map) % 2 == 0)
                baseChance = 0.08f; // 20% every minute, gives about a patient per 50 minutes  
            // 7 days to die ...
            if (GenLocalDate.DayOfQuadrum(map) == 6 || GenLocalDate.DayOfQuadrum(map) == 13)
                baseChance = 0.16f; // 40% every minute, gives about a patient per 10 minutes 
